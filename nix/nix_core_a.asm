    ...
    INCLUDE nix_core_a_inner.h


    ;栈8字节对齐
    PRESERVE8

    AREA    NIX_CORE_A, CODE, READONLY
    THUMB
    ...
    ;入口参数: none.
    ;返 回 值: none.
NIX_PendSvContextSwitch

    ;保存接口寄存器
    PUSH   {R14}

    ;调用C语言任务调度函数
    LDR    R0, =NIX_TaskSched       ;函数地址存入R0
    ADR.W  R14, {PC} + 0x7          ;保存返回地址
    BX     R0                       ;执行NIX_TaskSched函数

    ;保存当前任务的栈信息
    MOV    R14, R13                 ;将SP存入LR
    LDR    R0, =gpstrCurTaskReg     ;获取变量gpstrCurTaskReg的地址
    LDR    R12, [R0]                ;将当前任务寄存器组地址存入R12
    MOV    R0, R12	            ;将R12的值存入R0
    CBZ    R0, __BACKUP_REG	    ;R0若为0则直接跳转，不需要保存当前任务栈信息
